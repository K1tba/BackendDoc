...
# --
# ADD: Support Content-Type - multipart/form-data
# --
            elsif ( $Config->{ContentType} eq 'MULTIPART' ) {
...
# ADD: Support Content-Type - multipart/form-data
# --
            elsif ( $Headers->{'Content-Type'} eq 'multipart/form-data' ) {

                # Build multipart body
                my ( $MultipartBody, $Boundary ) = $Self->_BuildMultipartBody( Data => $Param{Data} );
                $Param{Data} = $MultipartBody;

                # Set proper Content-Type with boundary
                $Headers->{'Content-Type'} = "multipart/form-data; boundary=$Boundary";

                $SkipEncodeOutput = 1; # Важно: не применять кодировку к бинарным данным
            }
# --
            else {
...
#            $EncodeObject->EncodeOutput( \$Param{Data} );
            ...
            # Make sure data is correctly encoded.
            if ( !$SkipEncodeOutput && IsStringWithData( $Param{Data} ) ) {
                $EncodeObject->EncodeOutput( \$Param{Data} );
            }
# --
        }
...
sub _BuildMultipartBody {
    ...
    my $Data = $Param{Data};   # хеш с данными

    # 1. Отделяем файлы от остальных данных
    my %NonFileData = %$Data;
    my @FileParts;
    
    # Предполагаем, что файлы передаются в ключе 'Attachment' (массив)
    if ( exists $NonFileData{Attachment} && ref $NonFileData{Attachment} eq 'ARRAY' ) {
        for my $FileItem ( @{ $NonFileData{Attachment} } ) {
            if ( IsHashRefWithData($FileItem) && $FileItem->{Content} ) {
                push @FileParts, {
                    Filename    => $FileItem->{Filename}    || 'file',
                    ContentType => $FileItem->{ContentType} || 'application/octet-stream',
                    Content     => $FileItem->{Content},
                };
            }
        }
        delete $NonFileData{Attachment};
    }
    
    # 2. Преобразуем оставшиеся данные в плоскую структуру
    my %Flattened;
    $Self->_FlattenDataStructure(
        Data      => \%NonFileData,
        Flattened => \%Flattened,
    ...
    # 3. Генерируем границу
    my $Boundary = 'OTRSMultipartBoundary' . $Kernel::OM->Get('Kernel::System::Main')->GenerateRandomString();
    my $Body = '';
    
    # 4. Добавляем все плоские поля (текстовые)
    for my $Key ( sort keys %Flattened ) {
        my $Value = $Flattened{$Key};
