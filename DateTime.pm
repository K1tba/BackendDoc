...
sub new {
    ...
    # Use Time::Moment (for faster calculations - Delta) if it is installed.
    $Self->{TimeMoment} = $Kernel::OM->Get('Kernel::System::Main')->Require(
        'Time::Moment',
        Silent => 1,
    );
...
sub Delta {
    ...
    if ( !defined $Param{DateTimeObject} || ref $Param{DateTimeObject} ne ref $Self ) {
        $Kernel::OM->Get('Kernel::System::Log')->Log(
            'Priority' => 'Error',
    ...
    if ( $Param{ForWorkingTime} ) {
        ...
        my $StartTimeMomentObject;
        if ( $Self->{TimeMoment} ) {
            $StartTimeMomentObject = Time::Moment->from_object( $StartDateTimeObject );
        }
            ...
            )
        {
            ...
            eval {
                if ( $Self->{TimeMoment} ) {
                    ...
                    $StartTimeMomentObject->with_offset_same_instant(
                        $TimeZone->offset_for_datetime( $StartTimeMomentObject ) / 60
                    );
                }
                else {
        ...
        if ( $Self->{TimeMoment} ) {
            ...
            my @ShortDayNamesList = ('Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat');

            my $StartTime = $StartTimeMomentObject->epoch;
            my $StopTime  = Time::Moment->from_object( $Param{DateTimeObject}->{CPANDateTimeObject} )->epoch;
            ...
            while ( $StartTime < $StopTime ) {
                ...
                my $Year      = $StartTimeMomentObject->year;
                my $Month     = $StartTimeMomentObject->month;
                my $Day       = $StartTimeMomentObject->day_of_month;
                my $DayNumber = $StartTimeMomentObject->day_of_week;
                my $DayName   = $ShortDayNamesList[ $DayNumber % 7 ];
                my $Hour      = $StartTimeMomentObject->hour;
                my $Minute    = $StartTimeMomentObject->minute;
                my $Second    = $StartTimeMomentObject->second;
                ...
                if ( !$Hour && !$Minute && !$Second ) {
                    ...
                    # The following code is slightly faster than using CPAN DateTime's add(),
                    # presumably because add() always creates a DateTime::Duration object.
                    my $Epoch = $StartTimeMomentObject->epoch;
                    $Epoch += 60 * 60 * 24;

                    my $NextDayTimeMomentObject = Time::Moment->from_epoch( $Epoch );
                    $NextDayTimeMomentObject->with_offset_same_instant(
                        $TimeZone->offset_for_datetime( $NextDayTimeMomentObject ) / 60
                    );

                    # Only handle days with exactly 24 hours here
                    if (
                        !$NextDayTimeMomentObject->hour
                        && !$NextDayTimeMomentObject->minute
                        && !$NextDayTimeMomentObject->second
                        && $NextDayTimeMomentObject->day_of_month != $Day
                        ...
                        )
                    {
                        ...
                        if ( $FullDayProcessed ) {
                            ...
                            # Time implicitly set to 0
                            $StartTimeMomentObject = $StartTimeMomentObject->plus_days(1);
                ...
                $StartTimeMomentObject = Time::Moment->from_epoch( $Epoch )->with_offset_same_instant(
                    $TimeZone->offset_for_datetime( $StartTimeMomentObject ) / 60
                );
